...
class RedisService private constructor(
     ...
     */
    override suspend fun getAllSchedules(userId: String): Either<RedisError, List<Schedule>> {
        return validateUserId(userId).fold(
            { error -> error.left() },
            { userUuid ->
                Either.catch {
                    val pattern = "$keyPrefix:user:${userUuid}:schedule:*"
                    val keys = mutableListOf<String>()
     ...
     */
    override suspend fun getDailySchedule(userId: String): Either<RedisError, DailySchedule> {
        return either {
            ...
            val timeSlots = groupedByTime.map { (time, schedulesAtTime) ->
                val medicineItems = schedulesAtTime.mapNotNull { schedule ->
                    getMedicine(userId, schedule.medicineId.toString()).getOrNull()?.let { medicine ->
     ...
     */
    override suspend fun getAllDosageHistories(userId: String): Either<RedisError, List<DosageHistory>> {
        return validateUserId(userId).fold(
            { error -> error.left() },
            { userUuid ->
                Either.catch {
                    val pattern = "$keyPrefix:user:${userUuid}:dosagehistory:*"
                    val keys = mutableListOf<String>()
     ...
     */
    override suspend fun medicineExpiry(userId: String, now: java.time.LocalDateTime): Either<RedisError, List<MedicineWithExpiry>> = either {
        val medicines = getAllMedicines(userId).bind()
        val schedules = getAllSchedules(userId).bind()
        medicines
            .mapNotNull { medicine ->
